        ...
        try {
            ...
            if (tfactory.getFeature(SAXSource.FEATURE))
            {
                ...
                TransformerHandler handler1 = 
                    stfactory.newTransformerHandler(new StreamSource(new File("./src/test/org/portletbridge/xsl/test.xsl")));
                TransformerHandler handler2 = 
                    stfactory.newTransformerHandler(new StreamSource(new File("./src/test/org/portletbridge/xsl/testnext.xsl")));

                // Set up the parmeters. This paremeter will be passed to
                // stylesheet 1 (foo.xsl)
                handler1.getTransformer().setParameter("test", new Date());
                //parameters for stylesheet 2
                handler2.getTransformer().setParameter("test", new Date());
                ...
                Result result2 = new StreamResult(new OutputStreamWriter(System.out));
